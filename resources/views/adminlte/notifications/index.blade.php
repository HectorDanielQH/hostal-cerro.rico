    ...
    <x-adminlte-card icon="bi bi-bell" title="{{ __('adminlte.notifications') }}" bodyClass="p-0">
        ...
        <div class="list-group list-group-flush">
            @forelse ($notifications as $notification)
                <div class="list-group-item notification-row {{ $notification->read_at ? '' : 'unread' }}">
                    <span class="notification-icon">
                        <i class="{{ $notification->data['icon'] ?? 'bi bi-bell-fill' }} fs-4" aria-hidden="true"></i>
                    </span>
                    <div class="flex-grow-1">
                        <div class="fw-semibold notification-title">{{ $notification->data['title'] ?? __('adminlte.notifications') }}</div>
                        <div class="text-secondary notification-message">{{ $notification->data['message'] ?? '' }}</div>
                        <small class="text-secondary">
                            <i class="bi bi-clock me-1" aria-hidden="true"></i>{{ $notification->created_at?->diffForHumans() }}
                        </small>
                    </div>
