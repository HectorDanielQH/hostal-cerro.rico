...
class StoreReservationRequest extends FormRequest
{
    ...
    {
        return auth()->check() && auth()->user()->can('reservas.crear');
    }
    ...
    public function withValidator(Validator $validator): void
    {
        $validator->after(function (Validator $validator): void {
            ...
            if ($crossingReservationExists) {
                $validator->errors()->add('room_id', 'La habitacion ya tiene una reserva activa que se cruza con el rango de fechas seleccionado.');
            }
            ...
            if ((float) $this->input('initial_payment_amount', 0) > 0) {
                if (! $this->filled('initial_payment_currency')) {
                    $validator->errors()->add('initial_payment_currency', 'Selecciona la moneda del pago inicial.');
                }

                if (! $this->filled('initial_payment_method')) {
                    $validator->errors()->add('initial_payment_method', 'Selecciona el metodo del pago inicial.');
                }
            }
        });
    }
